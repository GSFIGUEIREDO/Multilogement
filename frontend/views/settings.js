(function () {
  ...
  window.ClimaParcSettingsView = {
    create(context) {
      ...
      function deleteSettingButton(collectionKey, itemId) {
        return `<button type="button" class="link-button danger-link" data-action="delete-setting-item" data-collection-key="${escapeHtml(collectionKey)}" data-id="${escapeHtml(itemId)}">Supprimer</button>`;
      }

      function settingsView() {
        const actions = `<button class="primary-button" data-action="open-modal" data-modal="dataField">Champ de données</button><button class="primary-button" data-action="open-modal" data-modal="serviceType">Type de demande</button><button class="ghost-button" data-action="open-modal" data-modal="interventionType">Type d'intervention</button><button class="ghost-button" data-action="open-modal" data-modal="formTemplate">Formulaire terrain</button><button class="ghost-button" data-action="open-modal" data-modal="hvacSystemType">Type de système HVAC</button><button class="ghost-button" data-action="open-modal" data-modal="storageLocation">Dépôt</button>`;
      ...
      function hvacSystemTypeModal(modal) {
        const type = state.hvacSystemTypes.find((item) => item.id === modal.id) || { topology: "split", active: true, sortOrder: state.hvacSystemTypes.length * 10 + 10 };
        return modalShell(type.id ? "Modifier le type de système HVAC" : "Nouveau type de système HVAC", `<form class="form-grid" data-form="hvacSystemType"><input type="hidden" name="id" value="${escapeHtml(type.id || "")}"><div class="split"><div class="field"><label>Nom</label><input name="name" value="${escapeHtml(type.name || "")}" required></div><div class="field"><label>Ordre</label><input name="sortOrder" type="number" min="0" value="${escapeHtml(type.sortOrder || 0)}"></div></div><div class="field"><label>Topologie</label><select name="topology"><option value="monobloc" ${type.topology === "monobloc" ? "selected" : ""}>Système unique (PTAC, TTW...)</option><option value="split" ${type.topology !== "monobloc" ? "selected" : ""}>Unité intérieure + unité extérieure</option></select></div><label class="check-row"><input type="checkbox" name="active" ${type.active !== false ? "checked" : ""}><span><strong>Type actif</strong></span></label><button class="primary-button" type="submit">Enregistrer</button></form>`);
      }
      ...
      async function saveHvacSystemType(form, values) {
        const payload = { id: values.id || `system_type_${slugify(values.name)}`, name: values.name, topology: values.topology || "split", sortOrder: Number(values.sortOrder || 0), active: Boolean(form.querySelector('[name="active"]')?.checked) };
        await saveSettingCollectionItem("hvacSystemTypes", payload, state.hvacSystemTypes.some((item) => item.id === payload.id) ? "Type de système modifié." : "Type de système créé.");
      }
      ...
      return {
        ...
        hvacSystemTypeModal, saveHvacSystemType, saveStorageLocation, settingsView, slugify, storageLocationModal
      };
    }
  };
})();
