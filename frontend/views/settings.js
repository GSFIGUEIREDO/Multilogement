  ...
  window.ClimaParcSettingsView = {
    create(context) {
      ...
      function settingsView() {
        ...
        return appShell(`${renderTopbar("Paramètres", "Types de demandes, checklists et droits d'accès.", actions)}
          <section class="grid"><div class="stack">
            ...
            <div class="panel"><div class="panel-header"><h2>Dépôts</h2></div><div class="panel-body cards-list">${state.storageLocations.length ? state.storageLocations.map((storage) => `<article class="list-item"><h3>${escapeHtml(storage.name)}</h3><div class="meta">${escapeHtml(state.clients.find((item) => item.id === storage.clientId)?.name || "-")} | ${escapeHtml(storage.address || "Adresse non précisée")} | ${storage.active === false ? "Inactif" : "Actif"}</div><div class="actions"><button class="ghost-button" data-action="open-modal" data-modal="storageLocation" data-id="${escapeHtml(storage.id)}">Modifier</button></div></article>`).join("") : `<div class="empty">Aucun dépôt enregistré.</div>`}</div></div>
      ...
      function storageLocationModal(modal) {
        const storage = state.storageLocations.find((item) => item.id === modal.id) || { active: true };
      ...
      async function saveStorageLocation(form, values) {
        const payload = { id: values.id || uid("storage"), name: values.name, clientId: values.clientId, address: values.address || "", active: Boolean(form.querySelector('[name="active"]')?.checked) };
