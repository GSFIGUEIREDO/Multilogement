  ...
  window.ClimaParcWorkOrdersView = {
    create(context) {
      ...
      function workOrderActionButtons(order, expanded) {
        ...
        return `<div class="actions"><button class="primary-button" data-action="execute-workorder" data-id="${escapeHtml(order.id)}">Exécuter</button>${order.equipmentId ? `<button class="ghost-button" data-action="open-checklist" data-id="${escapeHtml(order.id)}">Checklist</button>` : ""}${can("workorders") ? `<button class="ghost-button" data-action="open-modal" data-modal="workorder" data-id="${escapeHtml(order.id)}">Modifier</button><button class="ghost-button" data-action="order-status" data-id="${escapeHtml(order.id)}" data-status="termine">Terminer</button>` : ""}</div>`;
      }
      ...
      function workOrderExecutionView() {
        ...
        const canEdit = currentUser()?.role !== "client" && (can("workorders") || can("interventions"));
        return appShell(`${renderTopbar(`Execution ${order.number}`, `${building?.name || "-"} - ${type?.name || ""}`, `<button class="ghost-button" data-action="go-back" data-fallback-view="bons">Retour</button>${canEdit ? `<button class="ghost-button" data-action="open-modal" data-modal="workorder" data-id="${order.id}">Changer le formulaire</button><button class="primary-button" data-action="open-modal" data-modal="fieldIntervention" data-order="${order.id}" data-apartment="${selectedApartment?.id || ""}">Nouvelle activité</button>` : ""}`)}
          <section class="stats-grid"><div class="stat"><span>RDV</span><strong>${formatDate(order.scheduledDate)}</strong></div><div class="stat"><span>Progression</span><strong>${progress.percent}%</strong></div><div class="stat"><span>Appartements realises</span><strong>${progress.doneApartments}/${progress.totalApartments}</strong></div><div class="stat"><span>Machines analysees</span><strong>${progress.machines}</strong></div>${canEdit ? `<div class="stat"><span>Formulaire</span><strong>${escapeHtml(template?.name || "-")}</strong></div>` : ""}</section>
          ...
          <div class="panel-body cards-list">${machines.map((machine) => {
            ...
            return `<article class="list-item"><div class="actions" style="justify-content:space-between"><h3>${escapeHtml(machine.type)}</h3>${intervention ? statusBadge("terminee") : statusBadge("planifie")}</div><div class="meta">${escapeHtml(machine.brand)} ${escapeHtml(machine.model)} - ${escapeHtml(machine.location || "-")}</div>${canEdit ? `<div class="actions">${canCreateWorkOrders() ? `<button class="ghost-button" data-action="open-modal" data-modal="workorder" data-equipment="${machine.id}">Nouveau BT</button>` : ""}<button class="ghost-button" data-action="open-modal" data-modal="fieldIntervention" data-order="${order.id}" data-apartment="${selectedApartment?.id || ""}" data-equipment="${machine.id}">${intervention ? "Modifier le formulaire" : "Remplir le formulaire"}</button></div>` : ""}</article>`;
