    ...
    create(context) {
      ...
      function workOrderExecutionView() {
        ...
        const groupHtml = groups.map((group) => {
          ...
          const activityMenuItems = group.machines.map((machine) => `<button type="button" data-action="open-modal" data-modal="fieldIntervention" data-order="${escapeHtml(order.id)}" data-apartment="${escapeHtml(selectedApartment?.id || "")}" data-equipment="${escapeHtml(machine.id)}" data-system="${escapeHtml(group.system?.id || machine.systemId || "")}" data-activity-type="${escapeHtml(order.defaultActivityTypeId || order.typeId || "")}">Activité - ${escapeHtml(machine.model || machine.serial || unitPositionLabel(machine, topology))}</button>`).join("");
          const unitMenuItems = topology === "split" && group.system
            ? `<button type="button" data-action="open-modal" data-modal="fieldIntervention" data-order="${escapeHtml(order.id)}" data-apartment="${escapeHtml(selectedApartment?.id || "")}" data-system="${escapeHtml(group.system.id)}" data-unit-kind="interieure" data-activity-type="${escapeHtml(order.defaultActivityTypeId || order.typeId || "")}">Ajouter une unité intérieure</button>${exterior.length ? "" : `<button type="button" data-action="open-modal" data-modal="fieldIntervention" data-order="${escapeHtml(order.id)}" data-apartment="${escapeHtml(selectedApartment?.id || "")}" data-system="${escapeHtml(group.system.id)}" data-unit-kind="exterieure" data-activity-type="${escapeHtml(order.defaultActivityTypeId || order.typeId || "")}">Ajouter une unité extérieure</button>`}`
            : "";
          const actionMenu = canExecute && group.system && (unitMenuItems || activityMenuItems)
            ? `<details class="system-action-menu"><summary class="icon-button" title="Ajouter" aria-label="Ajouter une unité ou une activité">+</summary><div class="system-action-menu-list">${unitMenuItems}${activityMenuItems}</div></details>`
            : "";
          const systemBody = topology === "split"
            ? `<div class="system-machine-map"><div class="system-machine-row"><div class="system-unit-label"><span>EXT</span><strong>Unité extérieure</strong></div><div class="system-unit-list">${exterior.map(renderMachine).join("") || `<div class="system-placeholder">Unité extérieure à ajouter</div>`}</div></div><div class="system-machine-row"><div class="system-unit-label"><span>INT</span><strong>Unités intérieures</strong></div><div class="system-unit-list">${interiors.map(renderMachine).join("") || `<div class="system-placeholder">Unité intérieure à ajouter</div>`}</div></div></div>`
            : `<div class="system-machine-map"><div class="system-machine-row"><div class="system-unit-label"><span>1</span><strong>Système unique</strong></div><div class="system-unit-list">${group.machines.map(renderMachine).join("")}</div></div></div>`;
          return `<section class="hvac-system-group"><div class="system-group-header"><div><strong>${escapeHtml(group.system?.name || "Système à confirmer")}</strong><span>${escapeHtml(systemType?.name || group.machines[0]?.type || "-")} | ${escapeHtml(group.system?.brand || group.machines[0]?.brand || "Marque à confirmer")}</span></div>${actionMenu}</div>${systemBody}</section>`;
        }).join("");
