...
from src.climaparc.equipment.application.commands import CreateEquipmentCommand
...
class CreateEquipmentUseCase:
    ...
    def __call__(self, command: CreateEquipmentCommand) -> dict:
        ...
        if find_equipment_index(items, equipment["id"]) >= 0:
            raise ApplicationError("Machine existe deja.", HTTPStatus.CONFLICT)
