...
from src.climaparc.work_orders.application.commands import CreateWorkOrderCommand
...
class CreateWorkOrderUseCase:
    ...
    def __call__(self, command: CreateWorkOrderCommand) -> dict:
        ...
        if find_work_order_index(work_orders, work_order["id"]) >= 0:
            raise ApplicationError("Bon de travail existe deja.", HTTPStatus.CONFLICT)
